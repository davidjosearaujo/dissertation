...
cat /etc/open5gs/amf.yaml | yq ".amf.ngap.server[0].address = \"$CORE_IP\"" | sudo tee /etc/open5gs/amf.yaml > /dev/null
cat /etc/open5gs/upf.yaml | yq ".upf.gtpu.server[0].address = \"$CORE_IP\"" | sudo tee /etc/open5gs/upf.yaml > /dev/null

echo -e "\nAttributing DNN name to default APN"
cat /etc/open5gs/smf.yaml \
| yq '.smf.session[0].dnn = "backhaul"' \
| sudo tee /etc/open5gs/smf.yaml > /dev/null
cat /etc/open5gs/upf.yaml \
| yq '.upf.session[0].dnn = "backhaul"' \
| yq '.upf.session[0].dev = "ogstun"' \
| sudo tee /etc/open5gs/upf.yaml > /dev/null
...
}" | sudo tee /etc/nginx/sites-available/webui > /dev/null
...
sudo ip link set clientun0 up
sudo ip addr add 10.46.0.1/24 brd 10.46.0.255 dev clientun0
" | sudo tee /etc/init.d/clientun0 > /dev/null
sudo chmod +x /etc/init.d/clientun0