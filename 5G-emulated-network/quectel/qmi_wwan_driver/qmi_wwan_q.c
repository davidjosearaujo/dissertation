...
static uint __read_mostly bridge_mode = BIT(1) | BIT(2) | BIT(3) /*0|BIT(1)*/;
...
static int bridge_arp_reply(struct net_device *net, struct sk_buff *skb, uint bridge_ipv4)
{
	...
	if (parp->ar_hrd == htons(ARPHRD_ETHER) && parp->ar_pro == htons(ETH_P_IP) && parp->ar_op == htons(ARPOP_REQUEST) && parp->ar_hln == 6 && parp->ar_pln == 4)
	{
...
static struct rtnl_link_stats64 *_rmnet_vnd_get_stats64(struct net_device *net, struct rtnl_link_stats64 *stats)
{
	...
	if (nss_cb && dev->use_qca_nss)
	{ // rmnet_nss.c:rmnet_nss_tx() will update rx stats
		stats->rx_packets = 0;
		stats->rx_bytes = 0;
	}
...
static int qmi_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!status)
	{
		...
		if (dev->driver_info->flags & FLAG_NOARP)
		{
			...
			if (qmap_size > 4096 || dev->udev->speed >= USB_SPEED_SUPER)
			{ // if meet this requirements, must be LTE-A or 5G
				lte_a = 1;
			}
