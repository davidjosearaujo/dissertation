...
int enigen_catch_dump(uint8_t* pbuf, ssize_t size, const char *logfile_dir, const char* (*qlog_time_name)(int))
{
    ...
    if (g_is_eigen_chip == 1)  //EC600E/EC800E/EC800Z
    {
        for(k=0;k<size;k++)
        {
            if ((size - k) > 4 && (pbuf[k] == 0x22 && pbuf[k+1] == 0x0a && pbuf[k+2] == 0x45 && pbuf[k+3] == 0x80)) //EC618 platform
            {
                qlog_dbg("modem into ramdump ...\n");
                break;
            }
            
            if ((size - k) > 4 && (pbuf[k] == 0x22 && pbuf[k+1] == 0x04 && pbuf[k+2] == 0x02 && pbuf[k+3] == 0x00)) //EC718 platform
            {
                qlog_dbg("modem into ramdump ...\n");
                break;
            }
