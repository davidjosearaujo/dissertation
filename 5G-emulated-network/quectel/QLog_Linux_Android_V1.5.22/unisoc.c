...
struct ql_unisoc_ec800g_xml_data {
    struct ql_cmd ql_cmd_table[180];
    int ql_cmd_count;
};
...
{
    {
        ...
        {{{"region", "ap_qos", 0x52042100, 0x3c, 0}}},
        {{{"region", "cp_qos", 0x52043100, 0x3c, 0}}},
        ...
        {{{"region", "pp_mem", 0x1cc14000, 0x1000, 0}}},
        {{{"require", " ", 0x51707038, 0x1, 0x1}}},
        {{{"require", " ", 0x1c000004, 0x2, 0x2}}},
        {{{"region", "ae_link_mem", 0x1cd12000, 0x0620, 0}}},
        {{{"require", " ", 0x51707038, 0x1, 0x1}}},
        {{{"require", " ", 0x1c000004, 0x4, 0x4}}},
        {{{"region", "ae_ncs_mem", 0x1cd18000, 0x5000, 0}}},
        {{{"require", " ", 0x51707038, 0x1, 0x1}}},
        {{{"require", " ", 0x1c000004, 0x4, 0x4}}},
        {{{"region", "te_shared", 0x1ce40000, 0x30000, 0}}},
        {{{"require", " ", 0x51707038, 0x1, 0x1}}},
        {{{"require", " ", 0x1c000004, 0x8, 0x8}}},
        {{{"region", "ldpc_mem", 0x1cf3c000, 0x0320, 0}}},
        {{{"require", " ", 0x51707038, 0x1, 0x1}}},
        {{{"require", " ", 0x1c000004, 0x8, 0x8}}},
        {{{"region", "te_fifo", 0x1cf40000, 0xc000, 0}}},
        {{{"require", " ", 0x51707038, 0x1, 0x1}}},
        {{{"require", " ", 0x1c000004, 0x80, 0x80}}},
        {{{"region", "vitebi_symbol_mem", 0x1cf0c040, 0x09c0, 0}}},
        {{{"require", " ", 0x51707038, 0x1, 0x1}}},
        {{{"require", " ", 0x1c000004, 0x100, 0x100}}},
...
size_t unisoc_ec800g_send_cmd_complete(int fd, uint32_t addr, uint32_t size, int type)
{
    ...
    uint8_t buf_send_read32_tmp[32] = {0x7e, 0x02, 0x00, 0x00, 0xab, 0x18, 0x00, 0x0b, 0x00, 0x03, 0xa0, 0x10, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x80, 0x01, 0x00, 0x00, 0x00, 0x7e};
    uint8_t buf_send_read32[64] = {0x7e, 0x02, 0x00, 0x00, 0xab, 0x18, 0x00, 0x0b, 0x00, 0x03, 0xa0, 0x10, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x80, 0x01, 0x00, 0x00, 0x00, 0x7e};
    ...
    switch (type)
    {
        ...
        case 2:    //read32
        {
			memmove(buf_send_read32_tmp+1, &rid_ec800g, 4);
            memmove(buf_send_read32_tmp+17, &addr, 4);
            //memmove(buf_send_read32_tmp+21, &size, 4);
			for(i=0;i<26;i++)
			{
				if (i == 0 || i == (26 - 1))
				{
					buf_send_read32[j++] = buf_send_read32_tmp[i];
					continue;
				}

				if (buf_send_read32_tmp[i] == 0x7e)
				{
					buf_send_read32[j++] = 0x7d;
					buf_send_read32[j++] = 0x5e;
					send_size += 1;
				}
				else if (buf_send_read32_tmp[i] == 0x7d)
				{
					buf_send_read32[j++] = 0x7d;
					buf_send_read32[j++] = 0x5d;
					send_size += 1;
				}
				else
				{
    ...
    if (type == 0)
        ...
        return unisoc_ec800g_send_cmd(fd, buf_send_read32, send_size);
}

size_t unisoc_ec800g_recv_cmd_deal(int fd, uint8_t *pbuf, uint32_t* pvalue, uint16_t* psize, int type)
{
	size_t rc = 0;
	...
	do
	{
		...
		switch (type)
		{
			case 0:             //readBlock
			{
				...
				*psize = *((uint16_t*)buf_tmp) - 16;   //raw data
            ...
            case 1:             //readBlock
			{
				...
				*psize = *((uint16_t*)buf_tmp) - 16;   //raw data
                ...
                uint8_t buf_tmp1[4] = {0};    //value
                buf_tmp1[0] = buf_tmp1[real_start_offset + 25];
				buf_tmp1[1] = buf_tmp1[real_start_offset + 26];
				buf_tmp1[2] = buf_tmp1[real_start_offset + 27];
				buf_tmp1[3] = buf_tmp1[real_start_offset + 28];
                *pvalue = *((uint32_t*)buf_tmp1);
...
int unisoc_ec800g_catch_blue_screen(uint8_t* pbuf, const char *logfile_dir)
{
		...
		&& pbuf[22] == 0x00 && pbuf[23] == 0x00 && pbuf[24] == 0x00 && pbuf[25] == 0x01 && pbuf[26] == 0x7e)
    {
        ...
        unisoc_ec800g_recv_cmd_deal(unisoc_ec800g_blue_screen_fd, pbuf, &rvalue, &rsize, 0);
    ...
    for (i=0;i<ql_ec800g_xml_data_defult.ql_cmd_count;i++)
    {
        ...
        if (strStartsWith(ql_ec800g_xml_data_defult.ql_cmd_table[i].cmd.type, "region"))
        {
            ...
            else
            {
                ...
                if ((i+1) < ql_ec800g_xml_data_defult.ql_cmd_count && strStartsWith(ql_ec800g_xml_data_defult.ql_cmd_table[i+1].cmd.type, "require"))
                {
                    //get rvalue
                    unisoc_ec800g_send_cmd_complete(unisoc_ec800g_blue_screen_fd, ql_ec800g_xml_data_defult.ql_cmd_table[i+1].require.addr, 0x00, 2);
                    unisoc_ec800g_recv_cmd_deal(unisoc_ec800g_blue_screen_fd, pbuf, &rvalue, &rsize, 1);

                    //check whether to perform dump
                    if (!((rvalue & ql_ec800g_xml_data_defult.ql_cmd_table[i+1].require.mask) == ql_ec800g_xml_data_defult.ql_cmd_table[i+1].require.value))
                        continue;

                    if ((i+2) < ql_ec800g_xml_data_defult.ql_cmd_count && strStartsWith(ql_ec800g_xml_data_defult.ql_cmd_table[i+2].cmd.type, "require"))
                    {
                        //get rvalue1
                        unisoc_ec800g_send_cmd_complete(unisoc_ec800g_blue_screen_fd, ql_ec800g_xml_data_defult.ql_cmd_table[i+2].require.addr, 0x00, 2);
                        unisoc_ec800g_recv_cmd_deal(unisoc_ec800g_blue_screen_fd, pbuf, &rvalue1, &rsize, 1);

                        //check whether to perform dump
                        if (!((rvalue1 & ql_ec800g_xml_data_defult.ql_cmd_table[i+2].require.mask) == ql_ec800g_xml_data_defult.ql_cmd_table[i+2].require.value))
                            continue;
                    }
                }
